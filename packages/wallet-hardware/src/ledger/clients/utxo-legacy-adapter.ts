...
export async function extractInputsFromPsbt(tx: Transaction, chain: UTXOChain): Promise<UTXOType[]> {
  ...
  const utxoApi = getUtxoApi(chain);
  const getRawTx = createCachedRawTxResolver((txid) => utxoApi.getRawTx(txid));
  ...
  return Promise.all(
    Array.from({ length: tx.inputsLength }, async (_, inputIndex) => {
      ...
      if (!input.txid || input.index === undefined) {
        throw new Error(`PSBT input ${inputIndex} is missing txid/index`);
      }

      const txid = hex.encode(input.txid);
      const txHex = input.nonWitnessUtxo ? hex.encode(RawTx.encode(input.nonWitnessUtxo)) : await getRawTx(txid);
      if (!txHex) {
      ...
      const witnessUtxo = input.witnessUtxo
        ...
        : undefined;
      const nonWitnessPrevout = input.nonWitnessUtxo?.outputs?.[input.index];
      const value = witnessUtxo?.value ?? (nonWitnessPrevout ? Number(nonWitnessPrevout.amount) : 0);

      return { hash: txid, index: input.index, txHex, value, witnessUtxo } as UTXOType;
    }),
  );
}
