...
- [#159](https://github.com/swapkit/wallets/pull/159) [`2647ccb`](https://github.com/swapkit/wallets/commit/2647ccb8efe7fd6adf54f2b6ad57073a3300051e) Thanks [@hippocampusSK](https://github.com/hippocampusSK)! - **The keystore wallet is aligned with `@swapkit/wallet-keystore@6.0.0`.**
- [#159](https://github.com/swapkit/wallets/pull/159) [`2647ccb`](https://github.com/swapkit/wallets/commit/2647ccb8efe7fd6adf54f2b6ad57073a3300051e) Thanks [@hippocampusSK](https://github.com/hippocampusSK)! - **Built on SwapKit SDK 6 (`@swapkit/helpers`, `@swapkit/toolboxes`, `@swapkit/server` and `@swapkit/wallet-keystore` 6.0.0), which breaks public API these packages expose.**
- [#159](https://github.com/swapkit/wallets/pull/159) [`2647ccb`](https://github.com/swapkit/wallets/commit/2647ccb8efe7fd6adf54f2b6ad57073a3300051e) Thanks [@hippocampusSK](https://github.com/hippocampusSK)! - Extension wallets build their UTXO toolboxes with the script type of the address the extension returns (CTRL, Bitget, OKX, OneKey, KeepKey BEX and Vultisig), so a taproot or nested-segwit account is no longer reported as native segwit through `scriptType` and `getAddressFromKeys`. Without an address the toolbox keeps the chain default; an address of another chain or of a form the SDK cannot spend from (such as P2WSH) now fails the connect with the SDK's `toolbox_utxo_invalid_address` or `toolbox_utxo_unsupported_script_type`. Vultisig and CTRL now request the address before building the toolbox for every UTXO chain but Zcash.
- [#159](https://github.com/swapkit/wallets/pull/159) [`2647ccb`](https://github.com/swapkit/wallets/commit/2647ccb8efe7fd6adf54f2b6ad57073a3300051e) Thanks [@hippocampusSK](https://github.com/hippocampusSK)! - Ledger UTXO wallets build their toolbox with the script type of the address the device returns, so `scriptType` and `getAddressFromKeys` report the Ledger account's real type (BIP44 legacy, BIP49 nested SegWit or BIP84 native SegWit) instead of the chain default. Ledger signs by the path's wallet format, so a BIP86 path keeps its native SegWit address and is reported as such. An address the SDK cannot spend from now fails the connect with `toolbox_utxo_invalid_address` or `toolbox_utxo_unsupported_script_type`.
- [#159](https://github.com/swapkit/wallets/pull/159) [`2647ccb`](https://github.com/swapkit/wallets/commit/2647ccb8efe7fd6adf54f2b6ad57073a3300051e) Thanks [@hippocampusSK](https://github.com/hippocampusSK)! - The passkeys Bitcoin wallet builds its toolbox with the script type its payment address encodes, so `scriptType` and `getAddressFromKeys` report the account's real type (native segwit, nested segwit or taproot) instead of the chain default. Addresses and signing are unchanged.
- [#159](https://github.com/swapkit/wallets/pull/159) [`2647ccb`](https://github.com/swapkit/wallets/commit/2647ccb8efe7fd6adf54f2b6ad57073a3300051e) Thanks [@hippocampusSK](https://github.com/hippocampusSK)! - Update SwapKit SDK dependencies:
- [#159](https://github.com/swapkit/wallets/pull/159) [`2647ccb`](https://github.com/swapkit/wallets/commit/2647ccb8efe7fd6adf54f2b6ad57073a3300051e) Thanks [@hippocampusSK](https://github.com/hippocampusSK)! - Trezor UTXO wallets build their toolboxes with the script type the account path states (BIP44 legacy, BIP49 nested SegWit, BIP84 native SegWit), so `scriptType` and `getAddressFromKeys` report the Trezor account's real type instead of the chain default. Paths, addresses and device signing are unchanged.
- [#159](https://github.com/swapkit/wallets/pull/159) [`2647ccb`](https://github.com/swapkit/wallets/commit/2647ccb8efe7fd6adf54f2b6ad57073a3300051e) Thanks [@hippocampusSK](https://github.com/hippocampusSK)! - Follow the SDK's move to mainnet only, which removes `SKConfig` `envs.isStagenet`: WalletConnect always uses the NEAR mainnet namespace.
- [#156](https://github.com/swapkit/wallets/pull/156) [`9bf7e9e`](https://github.com/swapkit/wallets/commit/9bf7e9ef5b92ab204da471fce45aac139f56c1b3) Thanks [@hippocampusSK](https://github.com/hippocampusSK)! - Phantom: temporarily disable Bitcoin. Phantom deprecated the injected `window.phantom.bitcoin` provider and the Wallet Standard fallback no longer finds a Bitcoin signer, so connecting with every supported chain failed with `wallet_phantom_not_found` and took Ethereum, Monad and Solana down with it. Bitcoin is removed from Phantom's supported chains while the integration is reviewed; the Bitcoin connection code is kept. (via @swapkit/wallet-extensions@4.6.3)

## 4.13.1

### Patch Changes

- [#151](https://github.com/swapkit/wallets/pull/151) [`7abc5ef`](https://github.com/swapkit/wallets/commit/7abc5ef5be20ce512cf5ed13a7efa38e945dcc97) Thanks [@hippocampusSK](https://github.com/hippocampusSK)! - Add Arc (`Chain.Arc`, chain id 5042) to the Trezor connector. Trezor ships a signed network definition for chain 5042 (name `Arc`, symbol `USDC`, slip44 60), so the device shows the network and the native gas token correctly without any extra handling. (via @swapkit/wallet-hardware@4.11.0)
- [#152](https://github.com/swapkit/wallets/pull/152) [`b9f735c`](https://github.com/swapkit/wallets/commit/b9f735cd730385a752a32ccb5575e58c154a75d5) Thanks [@hippocampusSK](https://github.com/hippocampusSK)! - Ledger EVM: clear-sign ERC20 approvals on ARC, a chain the Ethereum app doesn't know yet. (via @swapkit/wallet-hardware@4.11.0)
...
- Updated dependencies [[`ef6fb67`](https://github.com/swapkit/wallets/commit/ef6fb67d7eaa92dbf0ec0362f3f28646fce9ebdc), [`db4f182`](https://github.com/swapkit/wallets/commit/db4f1820a8890abad0e956fcc45f87127c462478), [`0848590`](https://github.com/swapkit/wallets/commit/084859002565e8cd1f725ebfaa962dbe95d06609)]:
...
- Updated dependencies [[`bcafa97`](https://github.com/swapkit/wallets/commit/bcafa9727e3bf6f5e246950c486c41605cbcd7f0)]:
  - @swapkit/wallet-extensions@4.5.30
  - @swapkit/wallet-hardware@4.9.32
...
- Updated dependencies [[`67e51a3`](https://github.com/swapkit/wallets/commit/67e51a3adfcd9bda5ceb3fb5e8aed9b9f316bbe5)]:
  - @swapkit/wallet-extensions@4.5.27
  - @swapkit/wallet-hardware@4.9.28
...
- Updated dependencies [[`83cc493`](https://github.com/swapkit/wallets/commit/83cc49378a19588b49f4fe370c096f8f053969fd)]:
  - @swapkit/wallet-extensions@4.5.26
  - @swapkit/wallet-hardware@4.9.27
