...
export type ControlsStoreFieldValues = {
  ...
  // Extra request parameters merged into the /quote and /swap request bodies.
  // Raw JSON text as typed in the studio; parsed (and validated) at the point
  // of use. Studio-only escape hatch for testing backend request params —
  // offered only where dev tooling is enabled (non-production NODE_ENV builds
  // plus the deployed dev/stage studio hosts), and applied only while the
  // toggle is on. Requests keep going to the configured API / Dev API endpoints.
  quoteParams: string;
  swapParams: string;
