...
export interface WalletKeystoreConnectDialogProps {
  selectedChains?: Chain[];
}

export function WalletKeystoreConnectDialog({ selectedChains }: WalletKeystoreConnectDialogProps) {
  ...
  // Clear sensitive data when dialog unmounts
  useEffect(() => {
    return () => {
  ...
  const handleConnectWallet = form.handleSubmit(async (data) => {
    ...
    try {
      const { KEYSTORE_SUPPORTED_CHAINS } = await import("@swapkit/wallet-keystore");

      // Use selected chains if provided, otherwise use all supported chains
      ...
      // If filter results in empty (no overlap), fall back to all
      const finalChains = chainsToConnect.length > 0 ? chainsToConnect : KEYSTORE_SUPPORTED_CHAINS;

      await connectKeystore({ ...data?.keystoreFile, chains: finalChains }, data?.password);
  ...
  const handleKeystoreFileChange = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        captureWidgetError(error, { category: "wallet", tags: { action: "parse", provider: "keystore" } });
