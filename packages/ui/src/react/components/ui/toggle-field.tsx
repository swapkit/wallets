...
>({ label, control, name, description }: ToggleFieldProps<TFieldValues, TName>) {
  return (
    <FormField
      ...
      name={name}
      render={({ field }) => (
        <FormItem className="sk-ui-gap-1">
          {/* Label + switch share a single line; the description gets the full
              row width below so rows stay uniform instead of wrapping into a
              cramped column beside the switch. */}
          <div className="sk-ui-flex sk-ui-flex-row sk-ui-items-center sk-ui-justify-between sk-ui-gap-4">
