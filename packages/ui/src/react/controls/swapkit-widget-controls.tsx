...
// Extra request params are merged (spread) into request bodies, so the text
// must parse to a plain JSON object — arrays and scalars can't be spread.
function getParamsJsonError(value: string | undefined): string | null {
  ...
  try {
    ...
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      return 'Must be a JSON object, e.g. { "slippage": 3 }';
    }
    return null;
  } catch {
    return "Invalid JSON — the params are not sent until this parses.";
  }
}
...
export function SwapKitWidgetControls() {
  ...
  const quoteParamsError = getParamsJsonError(quoteParams);
  const swapParamsError = getParamsJsonError(swapParams);
      ...
      <Tabs className="sk-ui-flex-1 sk-ui-flex sk-ui-flex-col sk-ui-min-h-0" defaultValue="settings">
        ...
        <TabsContent
          ...
          <DeveloperSettingsSection
            ...
            localMode={localMode}
            quoteParamsError={quoteParamsError}
            requestParamsEnabled={requestParamsEnabled}
            swapParamsError={swapParamsError}
          />
        </TabsContent>
