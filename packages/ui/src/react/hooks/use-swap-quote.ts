...
export const useSwapQuote = ({ inputAsset, outputAsset, amount, enabled = true }: UseSwapQuoteParams) => {
  ...
  const { quoteParams: devQuoteParams } = useDevRequestParams();
  ...
  const fetchSwapQuote = useCallback(
    ...
    async (options?: { preserveSelection?: boolean }) => {
      ...
      try {
        ...
        // a pasted `sellAsset` can't desync the request from what's on screen.
  ...
  useDebouncedEffect(
    fetchSwapQuote,
    [amount, enabled, swapKit, swapKitConfig, outputAsset, inputAsset, devQuoteParams],
    { delay: 700, runImmediately: true },
  );
