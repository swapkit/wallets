...
import { resolveTokenSearchAssets } from "../use-token-search";
...
describe("filterAndSortAssets", () => {
  ...
  describe("token search merging", () => {
    test("keeps local order, deduplicates normalized identifiers, and appends api-only assets", () => {
      const assetsByChain = makeAssetsByChain(STATIC_IDENTIFIERS);
      const localResult = filterAndSortAssets({
        assetsByChain,
        balanceLookup: new Map(),
        options: { filterBySwapTo: false },
        searchQuery: "",
      ...
      const tokenSearchAssets = resolveTokenSearchAssets([
        {
          chainId: "eip155:1",
          decimals: 6,
        ...
        },
        {
          chainId: "eip155:1",
          decimals: 18,
          identifier: SWAPTO_ONLY_IDENTIFIER,
      ...
      expect(result.slice(0, localResult.length).map((asset) => asset.toString())).toEqual(
        localResult.map((asset) => asset.toString()),
      );
      expect(result.filter((asset) => asset.ticker === "USDC").length).toBe(1);
      expect(result.at(-1)?.toString()).toBe(SWAPTO_ONLY_IDENTIFIER);
    });

    test("uses api metadata for decimals, logos, and name searches", () => {
      const [tokenSearchAsset] = resolveTokenSearchAssets([
        {
          chainId: "eip155:1",
          decimals: 18,
          identifier: SWAPTO_ONLY_IDENTIFIER,
      ...
      expect(tokenSearchAsset?.asset.decimal).toBe(18);
      expect(tokenSearchAsset?.logoURI).toBe("https://example.com/uni.png");

      const result = filterAndSortAssets({
        assetsByChain: makeAssetsByChain(STATIC_IDENTIFIERS),
        balanceLookup: new Map(),
        options: { filterBySwapTo: false },
        searchQuery: "protocol",
        selectedNetworks: [Chain.Ethereum],
        swapToAssets: [],
        tokenSearchAssets: tokenSearchAsset ? [tokenSearchAsset] : [],
      });

      expect(result.map((asset) => asset.toString())).toEqual([SWAPTO_ONLY_IDENTIFIER]);
    });
  });
});
